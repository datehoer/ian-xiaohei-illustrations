...
用户选择更多图片、较少角色和完整场景时，可启用 [密集插画试验方案](ian-xiaohei-illustrations/references/dense-illustrated-video.md)。已制作的洗烘店视觉试片复用同一份已确认旁白，单独比较全屏画面、语义切镜与画风统一修订；新方案不自动取代旧版的用户确认记录。程序与复现步骤见 [试片说明](docs/dense-video-pilot.md)，每期源文件与成片按 [素材管理](docs/material-storage.md) 保存于仓库外。
