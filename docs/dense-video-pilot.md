...
本页说明可复用的布局接口与已有试片记录。下文的 `examples/video/` 和 `outputs/` 均位于仓库外的素材根目录；它们在本机通过忽略的软链接兼容旧路径，不随 clone 分发，详见 [素材管理](material-storage.md)。复现命令从代码仓库根目录执行，并显式指定外部素材与输出路径。
...
```bash
.venv/bin/python ../ian-xiaohei-video-projects/examples/video/laundromat-dense-pilot-v2/author.py
.venv/bin/python scripts/episode_video.py render --editions short --output ../ian-xiaohei-video-projects/outputs/laundromat-dense-pilot-v2-clean
```
...
```bash
.venv/bin/python ../ian-xiaohei-video-projects/examples/video/laundromat-dense-pilot/author.py
.venv/bin/python scripts/episode_video.py render --editions short --output ../ian-xiaohei-video-projects/outputs/laundromat-dense-pilot
```
...
图解与后期文字均可由代码修改，生成插图的原始文件另存。Git 保存规则、程序、必要角色参考与最小示例；本期内容源文件、生成图、完整成片及音频均保存在仓库外。恢复成片需要单独保存的完整素材与原旁白。
