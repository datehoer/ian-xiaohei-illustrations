...
更新：2026-09-11。上下文已发生一次系统压缩，当前最新修订为画风统一试片 v2。

## 最新目标与反馈

用户要求比较我们的洗烘店视频与 `/Volumes/MachineU1/chrome_downloads/The Economics of Owning a Cafe.mp4`，随后同意增加配图、减少柯基出镜、优化 Skill，并先做试片看效果。

用户认可第一版密集试片的衔接，但指出中途凸显柯基突兀，进一步明确“主要是风格也不相同，反正需要调整一下感觉”。本轮已经制作独立修订版，尚待用户看过后反馈，不能标为完全认可。

## 当前修订

- 最新成片：`outputs/laundromat-dense-pilot-v2/short/video.mp4`。
- 左右对比：`outputs/laundromat-dense-pilot-v2/comparison.mp4`，左上一密集试片，右画风统一修订，共用一条配音。
...
- 最终成片 SHA256：`2b3dc3ffe0fccaaba5f8187e03eebec207bc679313a7613667d730ed0862c738`。
...
运行 `.venv/bin/python examples/video/laundromat-dense-pilot-v2/author.py`，再运行 `.venv/bin/python scripts/episode_video.py render --editions short --output outputs/laundromat-dense-pilot-v2`。原确认版音频必须保留。输出中的 `verify_revision.py` 可重做技术检查和抽帧；它会将目检状态重置为待检查。
