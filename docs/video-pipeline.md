...
.venv/bin/python scripts/narrated_video.py examples/video/laundromat/manifest.json --output outputs/laundromat-pilot
```
...
```bash
python3 scripts/narrated_video.py examples/video/laundromat/manifest.json --output outputs/laundromat-pilot --offline
```
...
[示例清单](../examples/video/laundromat/manifest.json) 按字幕/数据揭示节点组织 `segments`，每段明确对应图片，图片路径相对清单所在目录。
...
用户确认中国自助洗烘店，并提出更频繁的配图变化及短片加长片形式。接口测试、来源核验和下一版制作建议见 [洗烘店数据与下一版方案](laundromat-next-version.md)。该文档中的新版功能仍是待实施方案。
