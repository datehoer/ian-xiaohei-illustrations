...
每期建立独立目录，例如 `examples/video/coffee-shop/`，保存 `short.json`、`long.json`、来源、账本、脚本、分镜、提示词和 `images/`。可参考 [短版结构](../examples/video/laundromat-v2/short.json) 与 [长版结构](../examples/video/laundromat-v2/long.json)。
...
.venv/bin/python scripts/episode_video.py prepare \
  examples/video/coffee-shop/short.json examples/video/coffee-shop/long.json \
...
| 每期可编辑内容 | `examples/video/题材/` | 替换数据、稿件、图片和分镜 |
| 确认版记录 | 本期 `approved-baseline.json` | 用户确认点、文件位置与 SHA-256 指纹 |
| 完整制作材料 | 本期 `outputs/题材/` | 原始 TTS 缓存、旁白、字幕、时间轴、检查记录及成片 |

Git 保存规则、程序、稿件和选定图片；若图片很多，可另采用素材存储。当前 `outputs/` 被 Git 忽略，提交仓库不会备份成片与配音。完整恢复需要另行备份本期 outputs，至少保留 `audio-cache/`、各版 `audio/`、`narration.wav`、`audio-timeline.json`、`captions.json`、SRT、镜头时间轴、检查记录和已确认 MP4。模型下载与中间渲染缓存可以重建；只备份 MP4 无法重做干净字幕和版式。
