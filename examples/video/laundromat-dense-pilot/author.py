"""Rebuild the visual trial while reusing the exact approved 60-second narration."""
import hashlib
import json
import os
...
OUT=REPO/'outputs/laundromat-dense-pilot/short'
INK='#263830';GREEN='#2F7166';ORANGE='#CF7135';RED='#B44B43';PAPER='#FEFDF9'
...
def main():
    ...
    m.update(profile='dense-illustrated-static-v1',disclosure='社区店 · 演示假设',status='visual-trial-awaiting-feedback')
    plans=[
        [shot('storefront',0,'进入社区洗烘店'),
         shot('owner',4,'用假设开始算账',[label('一年能剩多少？',.25,.39,72),label('跟着一筒衣服算',.25,.53,42,color=GREEN)],True)],
        ...
        [shot('rent',0,'租金仍然发生',left('−72,000 元','年度房租',color=RED,background='#FEFDF9')),
         shot('cleaning',2.38,'店主劳动有成本',left('−48,000 元','人工 · 已计店主劳动',color=RED,background='#FEFDF9'),True)],
        [shot('tools',0,'其他运营成本',left('−18,000 元','年度其他运营',color=RED)),
         ...
         shot('profit',3.66,'全年税前利润',left('57,600 元','年度税前利润',color=GREEN),True)],
        ...
        [shot('owner',0,'强调假设边界',[label('这是一组假设',.25,.37,74),label('不是行业均值',.25,.53,48,color=RED)],True),
         shot('storefront',2.82,'回到真实门店',[label('客流 · 成交价 · 房租',.5,.19,72,background='#FEFDF9')]),
         shot('profit',6.0,'真实条件决定结果',[label('最后剩多少',.24,.37,88),label('由真实门店条件决定',.24,.53,42,color=GREEN)],True)],
    ]
    ...
    lines=['# 洗烘店密集插画视觉试片','', '同一配音、同一账本；仅改变画面、标注与切镜。人物比例按实际镜头统计。','', '| 段落 / 段内起点 | 核心意思 | 画面 | 人物 |','|---|---|---|---|']
    for b in m['blocks']:
